...
function solution(money, product) {
    ...
    for(let i = 0; i<students; i++) {
        let nowmoney = money - (product[i][0]/2 + product[i][1]);
        let count = 1;
        for(let j =0; j<students; j++) {
            if(j!==i && (product[j][0] + product[j][1]) > nowmoney) break;
            if(j!==i && (product[j][0] + product[j][1]) <= nowmoney) {
                nowmoney -= (product[j][0] + product[j][1]);
                count++;
            }
        }
...
console.log(solution(money, [[6, 6],[2, 2],[4, 3],[4, 5],[10, 3]]))